...
<div align="left">
  ...
  Поэтому нужно использовать комбинированный метод расстояния между точками.
    ...
    - Для каждого заказа нужно найти все эвклидовы расстояния. Далее нужно взять 25 самых маленьких расстояний между курьером и заказом потом обратиться к API 2GIS и найти время в пути для каждого курьера до точки А. 
      Далее это подставить в матрицу (Венгерский алгоритм). Какие-то поля будут пустыми в матрице и лучше просто сохранить их в виде inf (бесконеность).
